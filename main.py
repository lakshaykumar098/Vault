...
from password_analyzer import analyze_password_strength, score_password, show_strength_bar
...
def check_password_expiry(credentials):
    ...
    for entry in credentials:
        ...
        if datetime.now() - created_at_dt > timedelta(days=90):
            ...
            if issues:
                ...
                while True:
                    ...
                    if choice == "1":
                        ...
                        strength = analyze_password_strength(user_password)
                        score = score_password(user_password)
                        show_strength_bar(score)
                        reused = password_reuse_check(user_password, key)
                        breached = check_breach(user_password)
